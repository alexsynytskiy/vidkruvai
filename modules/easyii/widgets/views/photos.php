...
$photoTemplate = '<tr data-id="{{photo_id}}">'.(IS_ROOT ? '<td>{{photo_id}}</td>' : '').'\
    ...
    <td>\
        Название <textarea class="form-control photo-description">{{photo_description}}</textarea>\
        Ссылка <input class="form-control photo-link" type="text" value="{{photo_link}}">\
        <a style="margin-top: 5px;" href="' . Url::to(['/admin/photos/description/{{photo_id}}']) . '" class="btn btn-sm btn-primary disabled save-photo-description">'. Yii::t('easyii', 'Save') .'</a>\
    </td>\
...
<table id="photo-table" class="table table-hover" style="display: <?= count($photos) ? 'table' : 'none' ?>;">
    ...
    <?php foreach($photos as $photo) : ?>
        <?= str_replace(
            ['{{photo_id}}', '{{photo_thumb}}', '{{photo_image}}', '{{photo_description}}', '{{photo_link}}'],
            [$photo->primaryKey, Image::thumb($photo->image, Photo::PHOTO_THUMB_WIDTH, Photo::PHOTO_THUMB_HEIGHT), $photo->image, $photo->description, isset($photo->link) ? $photo->link : ''],
            $photoTemplate)
        ?>
    <?php endforeach; ?>
    </tbody>
